...
fn judge_from_cmd(args: &[String]) -> bool{
    if args.len() > 3 && (&args[3]).contains("CASE_INSENSITIVE") { println!("{}",&args[3]);return false; }
    true
}
fn judge(args: &[String]) -> bool{
    match judge_from_cmd(args) {
        false => false,
        true => {
